...
Contained has two supported development entry points:

- `Contained.xcworkspace` for Xcode. It contains a native macOS app target that
  links the same package graph and builds/runs `Contained.app` directly.
- SwiftPM for command-line CI/release parity. `swift build`, `swift test`, and
  `Scripts/package.sh app` remain the release packaging path.
...
./Scripts/package.sh app debug # assemble a runnable development Contained.app
open Contained.app
```

`Scripts/package.sh app [debug|release]` compiles the executable and assembles the `.app` (Info.plist, icon, embedded Sparkle). The `.app` is a build artifact and is git-ignored.
