...
struct CustomizeWidgetsPanel: View {
    ...
    private func widgetOrderControls(_ index: Int) -> some View {
        ...
        return UI.Panel.Row(title: AppText.string("customize.widget.order", defaultValue: "Order"),
    ...
    private func widgetDisplayOptions(_ index: Int) -> some View {
        ...
        UI.Panel.ToggleRow(title: AppText.string("customize.widget.showIcon", defaultValue: "Show icon"), isOn: widgetBinding(index, \.showIcon))
    ...
    private func widgetDataOptions(_ index: Int) -> some View {
        ...
        UI.Panel.Row(title: AppText.string("customize.widget.metric", defaultValue: "Metric")) {
        ...
        if widgetStyle(index).requiresSecondaryMetric {
            if let fallback = secondaryMetricFallback(for: index) {
                UI.Panel.Row(title: AppText.string("customize.widget.compare", defaultValue: "Compare")) {
    ...
    private func widgetChartOptions(_ index: Int) -> some View {
        ...
        UI.Panel.Row(title: AppText.string("customize.widget.type", defaultValue: "Type")) {
            Picker("", selection: widgetStyleBinding(index)) {
                ForEach(UI.Chart.GraphStyle.allCases) { Text($0.localizedDisplayName).tag($0) }
            }
            .labelsHidden()
            .fixedSize()
        }
        if chartStyle == .area {
            UI.Panel.ToggleRow(title: AppText.string("customize.widget.gradientFill", defaultValue: "Gradient fill"), isOn: widgetBinding(index, \.areaUsesGradient))
        }
        if chartStyle.usesLineOptions {
            UI.Panel.Row(title: AppText.string("customize.widget.interpolation", defaultValue: "Interpolation")) {
                Picker("", selection: widgetBinding(index, \.interpolation)) {
                    ForEach(UI.Chart.Interpolation.allCases) { Text($0.localizedDisplayName).tag($0) }
                }
                .labelsHidden()
                .fixedSize()
            }
    ...
    private func widgetGroupLabel(_ title: String, systemImage: String) -> some View {
        UI.State.InlineStatus(title, systemImage: systemImage)
    }
    ...
    private func widgetStyle(_ index: Int) -> UI.Chart.GraphStyle {
    ...
    private func widgetStyleBinding(_ index: Int) -> Binding<UI.Chart.GraphStyle> {
        Binding {
            style.widget(at: index).style
        } set: { newValue in
            var widget = style.widget(at: index)
