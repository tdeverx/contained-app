...
struct ContainerCard: View {
    ...
    private var cardPages: UI.Card.Pages<Tab> {
        UI.Card.Pages(items: pageControlItems,
                          selection: tab,
                          tint: tint,
    ...
    private var statusChip: some View {
        UI.Card.FooterMini {
            if styleForDisplay.showStatusIcon {
                UI.Symbol.Image(systemName: statusSymbol,
                             tone: statusTone,
                             size: .caption2)
            }
        } text: {
            if styleForDisplay.showStatusText {
                UI.Card.MetricText(text: statusLabel)
    ...
    private var statusTone: UI.State.Tone {
        switch statusState {
    ...
    private var pageControlItems: [UI.Card.Page<Tab>] {
        Tab.allCases.map { item in
            UI.Card.Page(id: item,
                                        title: item.rawValue,
