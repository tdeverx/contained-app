...
struct ToolbarTemplatesPanel: View {
    ...
    var body: some View {
        UI.Panel.Scaffold(width: UI.Panel.Size.templates.width) {
            if showsHeader {
    ...
    private var header: some View {
        UI.Panel.Header(symbol: "bookmark",
    ...
    private var emptyCard: some View {
        UI.Card.Scaffold(size: .small,
            ...
            UI.Card.IconChip(symbol: "bookmark",
                                 tint: .secondary,
    ...
    private func templateCard(_ template: Template) -> some View {
        UI.Card.Scaffold(size: .medium,
            ...
            UI.Card.FooterMini {
                UI.Symbol.Image(systemName: "bookmark", size: .caption2)
            } text: {
                UI.Card.MetricText(text: "Saved run configuration")
            }
        } footerActions: {
            ...
            UI.Action.TextButton(title: AppText.string("common.use", defaultValue: "Use"),
...
/// surface is rendered by `UX.Morph.Expander`, so the source can hide while the panel owns the glass.
