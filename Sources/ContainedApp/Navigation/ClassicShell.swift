...
struct ClassicShell: View {
    ...
    private var detailColumn: some View {
        ...
        .overlay {
            if ui.toolbarUIEnabled {
                ...
                AppToolbar()
                    .environment(\.morphSafeAreas, toolbarSafeAreas)
    ...
    private var detailPage: some View {
        let insets = toolbarSafeAreas.insets(MorphSafeAreaPolicy(excluding: .top, padding: .none))
    ...
    private var toolbarSafeAreas: MorphSafeAreaManager {
        MorphSafeAreaManager(system: EdgeInsets(),
...
private struct NetworksPage: View {
    ...
    private func networkRow(_ network: NetworkResource) -> some View {
        DesignCard(size: .medium,
            ...
            DesignCardIconChip(symbol: network.isBuiltin ? "network.badge.shield.half.filled" : "network",
            ...
            if network.isBuiltin {
                DesignCardFooterMini {
                    ...
                    DesignCardMetricText(text: "Built-in")
                }
            }
        } footerActions: {
            EmptyView()
...
private struct ImagesPage: View {
        ...
        GeometryReader { viewport in
            ZStack {
                ...
                if let detail, presented {
                    ...
                    .environment(\.morphSafeAreas, imageDetailSafeAreas)
                    .zIndex(10)
                }
            }
    ...
    private var imageDetailSafeAreaPolicy: MorphSafeAreaPolicy {
        ui.toolbarUIEnabled ? MorphSafeAreaPolicy(excluding: .both, padding: .small) : .content
    }

    private var imageDetailSafeAreas: MorphSafeAreaManager {
        guard ui.toolbarUIEnabled else { return MorphSafeAreaManager(system: EdgeInsets()) }
        return MorphSafeAreaManager(system: EdgeInsets(),
                                  topToolbarHeight: AppToolbar.bandHeight,
                                  bottomToolbarHeight: AppToolbar.bandHeight)
    }
