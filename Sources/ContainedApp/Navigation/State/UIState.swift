...
final class UIState {
    ...
    struct SearchPresentation {
        /// Filter text applied by section list views and the command palette.
        var text = ""
    ...
    func toggleMorph(_ morph: ToolbarMorph) {
        if toolbar.activeMorph == morph {
            requestMorphClose(morph)
        } else {
            toolbar.activeMorph = morph
        }
    }

    func requestMorphClose(_ morph: ToolbarMorph? = nil) {
        guard let activeMorph = toolbar.activeMorph, morph == nil || activeMorph == morph else { return }
        toolbar.closeRequestToken &+= 1
    }
    ...
    func openCreationPanel(entry: CreationEntry = .menu,
        ...
        creation.returnEntry = returnEntry
        creation.searchQuery = searchQuery
        creation.requestToken &+= 1
    ...
    func advancePrefillQueue() {
        guard !prefill.queue.isEmpty else { return }
        let next = prefill.queue.removeFirst()
