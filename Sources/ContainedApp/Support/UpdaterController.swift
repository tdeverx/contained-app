...
private final class UpdaterBundleToken: NSObject {}
...
final class UpdaterController {
    ...
    static func changelogResourceURL(bundle: Bundle = .main) -> URL? {
        if let directResource = bundle.url(forResource: "CHANGELOG", withExtension: "md") {
            return directResource.absoluteURL.standardizedFileURL
        }
        if let resourceDirectory = bundle.resourceURL,
           let containedResource = resourceBundleChangelog(in: resourceDirectory) {
            return containedResource
        }
        if let siblingResource = siblingResourceBundleChangelog(near: bundle.bundleURL) {
        ...
        if bundle.bundleURL.pathExtension == "app" {
            ...
            for bundleName in ["Contained_ContainedApp.bundle", "Contained_Contained.bundle"] {
                for candidate in resourceBundleCandidates(at: resources.appendingPathComponent(bundleName)) {
                    if FileManager.default.fileExists(atPath: candidate.path) {
                        return candidate.absoluteURL.standardizedFileURL
                    }
                }
            }
            let directCandidate = resources.appendingPathComponent("CHANGELOG.md")
            if FileManager.default.fileExists(atPath: directCandidate.path) {
                return directCandidate.absoluteURL.standardizedFileURL
            }
        }
    ...
    private static func loadedResourceBundleChangelog() -> URL? {
        for bundle in Bundle.allBundles + Bundle.allFrameworks {
            if ["Contained_ContainedApp.bundle", "Contained_Contained.bundle"].contains(bundle.bundleURL.lastPathComponent),
               let resource = bundle.url(forResource: "CHANGELOG", withExtension: "md") {
                return resource.absoluteURL.standardizedFileURL
            }
            if let resourceURL = bundle.resourceURL,
               let resource = resourceBundleChangelog(in: resourceURL) {
                return resource
            }
        }
        return nil
    ...
    private static func siblingResourceBundleChangelog(near url: URL) -> URL? {
        let directory = url.deletingLastPathComponent()
        for bundleName in ["Contained_ContainedApp.bundle", "Contained_Contained.bundle"] {
            for candidate in resourceBundleCandidates(at: directory.appendingPathComponent(bundleName)) {
                if FileManager.default.fileExists(atPath: candidate.path) {
                    return candidate.absoluteURL.standardizedFileURL
                }
            }
    ...
    private static func resourceBundleChangelog(in directory: URL) -> URL? {
        for bundleName in ["Contained_ContainedApp.bundle", "Contained_Contained.bundle"] {
            for candidate in resourceBundleCandidates(at: directory.appendingPathComponent(bundleName)) {
                if FileManager.default.fileExists(atPath: candidate.path) {
                    return candidate.absoluteURL.standardizedFileURL
    ...
    private static func resourceBundleCandidates(at bundleURL: URL) -> [URL] {
        [
            bundleURL.appendingPathComponent("CHANGELOG.md"),
            bundleURL
                .appendingPathComponent("Contents")
                .appendingPathComponent("Resources")
