...
private struct ContainerHistoryWindow: View {
                ...
                chartCard("Memory", unit: percentUnit) {
                    Chart(chartPoints) { point in
                        AreaMark(x: .value("Time", point.timestamp),
                            ...
                            .foregroundStyle(Color.accentColor.opacity(DesignTokens.Chart.areaOpacity))
                    }
                    .percentHistoryScale()
                }
    ...
    private func chartCard<C: View>(_ title: String, unit: String, @ViewBuilder chart: @escaping () -> C) -> some View {
        ContainerTabSection {
            ...
            chart()
                .frame(height: DesignTokens.Chart.height)
                .chartXAxis { AxisMarks(values: .automatic(desiredCount: DesignTokens.Chart.axisDesiredCount)) }
...
struct EventRow: View {
    ...
    var body: some View {
        DesignCard(size: .small,
                     ...
                     title: event.message,
                     subtitle: subtitle) {
            DesignCardIconChip(symbol: event.kind.symbol,
                                 tint: event.kind.tint,
                                 backgroundOpacity: DesignTokens.DesignCard.iconEmphasisBackgroundOpacity)
        } titleAccessory: {
            EmptyView()
        } subtitleAccessory: {
            DesignBadgeText(text: event.kind.rawValue.capitalized)
        } headerAccessory: {
            if isUnread {
