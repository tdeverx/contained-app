...
final class AppModel {
    ...
    func bootstrapIfNeeded() async {
        ...
        case .cliMissing:
            ...
            logger.record("Container CLI missing", category: .system, severity: .error)
            return
        case .unsupported(let orchestrator, let url, let version):
            client = orchestrator
            cliURL = url
            cliVersion = version
            containers.client = orchestrator
            bootstrap = .unsupported(version: version)
            logger.record("Unsupported container CLI version \(version)", category: .system, severity: .error)
            return
        case .ready(let orchestrator, let url, let version):
            client = orchestrator
            cliURL = url
            cliVersion = version
            containers.client = orchestrator
