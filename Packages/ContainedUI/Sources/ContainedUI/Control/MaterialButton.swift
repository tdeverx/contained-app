...
private struct MaterialButtonItemHoverEnabledKey: EnvironmentKey {
    static let defaultValue = true
}

private extension EnvironmentValues {
    var materialButtonItemHoverEnabled: Bool {
        get { self[MaterialButtonItemHoverEnabledKey.self] }
        set { self[MaterialButtonItemHoverEnabledKey.self] = newValue }
    }
}
...
/// 4pt padding. Place it inside `MaterialButton` to get the full 36pt material capsule.
struct MaterialButtonItem<Label: View>: View {
    ...
    @Environment(\.materialButtonItemHoverEnabled) private var hoverEnabled
    ...
    private var content: some View {
        label()
            ...
            .frame(width: isIcon ? UI.Tokens.Toolbar.buttonItemHeight : nil,
...
/// Input content that occupies the same 28pt inner lane as `MaterialButtonItem`, but leaves hover/pressed
/// treatment to the enclosing `MaterialButton` container.
struct MaterialButtonInputItem<Content: View>: View {
    var spacing = UI.Tokens.Toolbar.searchIconGap
    @ViewBuilder var content: () -> Content

    init(spacing: CGFloat = UI.Tokens.Toolbar.searchIconGap,
...
extension MaterialButtonItem where Label == Image {
...
struct MaterialButton<Content: View>: View {
    var spacing: CGFloat = 0
    var height: CGFloat = UI.Tokens.Toolbar.buttonGroupHeight
    ...
    var body: some View {
        ...
        HStack(spacing: spacing) { content() }
            .padding(.horizontal, UI.Tokens.Toolbar.iconInnerPadding)
            .frame(height: height)
