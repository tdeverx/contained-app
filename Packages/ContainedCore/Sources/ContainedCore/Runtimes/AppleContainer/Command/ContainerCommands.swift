...
enum ContainerCommands {
    ...
    static func list(all: Bool) -> [String] {
        var args = ["list"]
        if all { args.append("--all") }
        return args + jsonFormat
    }

    static func stats(ids: [String] = [], noStream: Bool = true, format: StatsFormat = .json) -> [String] {
        var args = ["stats"]
        if noStream { args.append("--no-stream") }
        return args + ["--format", format.rawValue] + ids
    }

    static func statsTableStream(ids: [String] = []) -> [String] {
    ...
    static func copy(source: String, destination: String) -> [String] { ["copy", source, destination] }
    static func logs(_ id: String, follow: Bool = false, tail: Int? = nil, boot: Bool = false) -> [String] {
        var args = ["logs"]
    ...
    static func imageList() -> [String] { ["image", "list"] + jsonFormat }
    static func imageInspect(_ refs: [String]) -> [String] { ["image", "inspect"] + refs }
    static func imageDelete(_ refs: [String]) -> [String] { ["image", "delete"] + refs }
    ...
    static func imageLoad(input: String) -> [String] {
    ...
    static func networkList() -> [String] { ["network", "list"] + jsonFormat }
    static func volumeList() -> [String] { ["volume", "list"] + jsonFormat }
    ...
    /// `volume delete <names...>`
    static func volumeDelete(_ names: [String]) -> [String] { ["volume", "delete"] + names }
    ...
    static func networkDelete(_ names: [String]) -> [String] { ["network", "delete"] + names }
    ...
    static func registryList() -> [String] { ["registry", "list"] + jsonFormat }
    ...
    static func registryLogout(server: String) -> [String] { ["registry", "logout", server] }

    // MARK: System

    static let systemStatus = ["system", "status"] + jsonFormat
    static let systemDF = ["system", "df"] + jsonFormat
    static let systemPropertyList = ["system", "property", "list"] + jsonFormat
    static let version = ["--version"]
    /// `system logs [--follow] [--last N]` — service logs (plain text, not JSON).
    static func systemLogs(follow: Bool = false, last: Int? = nil) -> [String] {
    ...
    static let systemDNSList = ["system", "dns", "list"] + jsonFormat
    ...
    static func systemDNSDelete(_ domain: String) -> [String] { ["system", "dns", "delete", domain] }
    /// `system kernel set --recommended` — download + install the recommended kernel.
    static let systemKernelSetRecommended = ["system", "kernel", "set", "--recommended"]
}
