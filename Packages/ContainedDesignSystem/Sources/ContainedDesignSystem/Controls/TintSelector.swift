...
/// A row of colored swatches for picking a `DesignTint` — each shows its actual color, the selected
/// one gets a ring.
public struct TintSelector: View {
    private let selection: Binding<DesignTint?>
    private let automaticLabel: String?
    private let labelForTint: (DesignTint) -> String

    public init(selection: Binding<DesignTint>,
                labelForTint: @escaping (DesignTint) -> String) {
        self.selection = Binding<DesignTint?>(
    ...
    public init(optionalSelection: Binding<DesignTint?>,
    ...
    public var body: some View {
        HStack(spacing: DesignTokens.Space.s) {
            ...
            ForEach(DesignTint.allCases) { tint in
    ...
    private func swatch(_ tint: DesignTint) -> some View {
        ZStack {
